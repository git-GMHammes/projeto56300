...
import { useGroupMemberViewModel } from '../hooks/useGroupMemberViewModel'
...
export default function GroupMemberScreen({ groupId, goBack }: Props) {
  const { theme } = useTheme()
  const vm = useGroupMemberViewModel(groupId)
      ...
      <View style={styles.content}>
        ...
        {!vm.loading && vm.error === null && (
          <Text style={[styles.info, { color: theme.colors.text }]}>
            {vm.data.length} membros no grupo
          </Text>
        )}
      </View>
    </SafeAreaView>
