...
import { useGroupMessageViewModel } from '../hooks/useGroupMessageViewModel'
...
export default function GroupMessageScreen({ groupId, goBack }: Props) {
  const { theme } = useTheme()
  const vm = useGroupMessageViewModel(groupId)
      ...
      <View style={styles.content}>
        ...
        {!vm.loading && vm.error === null && (
          <Text style={[styles.info, { color: theme.colors.text }]}>
            {vm.data.length} mensagens no grupo {groupId}
          </Text>
        )}
      </View>
    </SafeAreaView>
