    ...
    return (
      <SafeAreaView style={styles.safe}>
        <View style={styles.centered}>
          ...
          <Text style={styles.successTitle}>Usuário criado!</Text>
          <Text style={styles.successSub}>ID: {vm.userId}</Text>
          <TouchableOpacity style={styles.btn} onPress={() => navigation.goBack()}>
            <Text style={styles.btnText}>Voltar à lista</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    )
          ...
          <View style={styles.card}>
            ...
            {vm.step === 'access' && (
              <>
                ...
                <InputField name="username" label="Usuário" value={vm.access.username} placeholder="nome.sobrenome" required allowLetters allowNumbers allowSpecial onChange={(_, v) => vm.setAccessField('username', v)} onValidationChange={() => {}} />
                <View style={styles.gap} />
                <PasswordField name="password" label="Senha" value={vm.access.password} placeholder="mínimo 6 caracteres" required minLength={6} onChange={(_, v) => vm.setAccessField('password', v)} onValidationChange={() => {}} />
                <View style={styles.gap} />
                <PasswordField name="confirmPassword" label="Confirmar Senha" value={vm.access.confirmPassword} placeholder="repita a senha" required doubleField confirmValue={vm.access.password} onChange={(_, v) => vm.setAccessField('confirmPassword', v)} onValidationChange={() => {}} />

                <TouchableOpacity style={[styles.btn, !vm.isAccessValid && styles.btnDisabled]} disabled={!vm.isAccessValid} activeOpacity={0.8} onPress={vm.goToProfile}>
            ...
            {vm.step === 'profile' && (
              <>
                ...
                <InputField name="name" label="Nome completo" value={vm.profile.name} placeholder="Seu Nome" required allowLetters allowSpecial onChange={(_, v) => vm.setProfileField('name', v)} onValidationChange={() => {}} />
                <View style={styles.gap} />
                <EmailField name="mail" label="E-mail" value={vm.profile.mail} placeholder="seu@email.com" required onChange={(_, v) => vm.setProfileField('mail', v)} onValidationChange={() => {}} />
                <View style={styles.gap} />
                <CpfField name="cpf" label="CPF" value={vm.profile.cpf} placeholder="000.000.000-00" required onChange={(_, v) => vm.setProfileField('cpf', v)} onValidationChange={() => {}} />
                <View style={styles.gap} />
                <PhoneField name="whatsapp" label="WhatsApp" value={vm.profile.whatsapp} placeholder="(00) 00000-0000" required onChange={(_, v) => vm.setProfileField('whatsapp', v)} onValidationChange={() => {}} />
                <View style={styles.gap} />
