...
namespace App\Services\V1\Msg\TimelineReaction;

use App\Models\V1\Msg\TimelineReaction\SqlTableModel;
use App\Models\V1\Msg\TimelineReaction\SqlViewModel;
...
class Processor extends MsgBaseService
{
    ...
    protected function validateOnCreate(array $data): ?array
    {
        ...
        if (!empty($data['timeline_id']) && !empty($data['user_management_id'])
            && $this->reactionExists((int) $data['timeline_id'], (int) $data['user_management_id'])
        ) {
    ...
    {
        unset($data['timeline_id'], $data['user_management_id']);
